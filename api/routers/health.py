...
class ScannerHealthItem(BaseModel):
    ...
    scanner_status: Optional[str] = None      # active | idle | offline
...
async def bulk_update_scanner_health(
    ...
    for item in items:
        ...
        # Always write scanner_status; only update last_heartbeat if we have a timestamp
        status_val = item.scanner_status or ("active" if item.is_online else "offline")
        try:
            if last_hb is not None:
                await db.execute(
                    text("UPDATE mst_scanner SET last_heartbeat = :ts, scanner_status = :status WHERE UPPER(mac_id) = :mac"),
                    {"ts": last_hb, "mac": mac, "status": status_val},
                )
            else:
                # Offline scanner — update status only, keep last heartbeat timestamp
                await db.execute(
                    text("UPDATE mst_scanner SET scanner_status = :status WHERE UPPER(mac_id) = :mac"),
                    {"mac": mac, "status": status_val},
                )
            updated += 1
        except Exception:
            pass
