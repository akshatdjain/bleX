...
@router.post("")
async def create_zone(payload: ZoneIn, db: AsyncSession = Depends(get_tenant_db)):
...
@router.put("/{zone_id}")
async def update_zone(zone_id: int, payload: ZoneIn, db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_zone(zone_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def assign_scanner_to_zone(
    zone_id: int, payload: ZoneScannerIn, db: AsyncSession = Depends(get_tenant_db)
):
...
async def unassign_scanner_from_zone(
    zone_id: int, scanner_id: int, db: AsyncSession = Depends(get_tenant_db)
):
    await db.execute(
