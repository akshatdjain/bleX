...
# -------------------------------------------------
scanner_last_seen   = {}   # scanner_mac (upper) -> last heartbeat timestamp (alive)
scanner_last_active = {}   # scanner_mac (upper) -> last timestamp when beacon_count > 0 (active)
...
def compute_zone_scores(zones: dict):
    ...
    for zone_id, scanners in zones.items():
        ...
        if values:
            scores[zone_id] = max(values)  # strongest scanner wins
...
def on_message(client, userdata, msg):
    ...
    if payload.get("type") == "heartbeat":
        hb_scanner = payload.get("scanner_id", "").upper()
        if payload.get("beacon_count", 0) > 0 and hb_scanner:
            scanner_last_active[hb_scanner] = time.time()
        return
