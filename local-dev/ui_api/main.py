...
# The main app serves static files. A SEPARATE sub-app serves the API so that
# the static file mount at '/' doesn't swallow /api/docs.
api = FastAPI(title="BleX UI API", version="2.0.0")
...
# API ROUTES on the sub-app
api.include_router(assets.router)
api.include_router(scanners.router)
api.include_router(history.router)
api.include_router(zones.router)
api.include_router(health.router)
api.include_router(notifications.router)

# Mount the API sub-app at /api — docs will be at /api/docs
app.mount("/api", api)

# SERVE UI (built Vite dist goes in www/) — MUST be last
