...
import { getAssetsForZone, getScannersForZone, type Zone, type Scanner } from "@/lib/data";
...
export function ZoneCard({ zone, index }: ZoneCardProps) {
  const assets = getAssetsForZone(zone.id);
  const scanners = getScannersForZone(zone.id);
          ...
          <CardContent className="p-5">
            ...
            <div className="flex items-start justify-between mb-1">
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <span className="tabular-nums">{zone.movementCount}</span>
              </div>
            </div>
                  ...
                  assets.map((asset) => (
                    <Tooltip key={asset.id}>
                      <TooltipTrigger asChild>
                        ...
                        >
                          <BeaconIcon shape={asset.shape} status={asset.status} size={22} />
