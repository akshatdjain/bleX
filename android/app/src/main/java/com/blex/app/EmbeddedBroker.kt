...
class EmbeddedBroker(private val context: Context) {
    ...
    fun start() {
        ...
        try {
            ...
            val moquetteDir = context.filesDir.resolve("moquette")
            val storePath = moquetteDir.absolutePath

            // Ensure storage directory exists
            moquetteDir.mkdirs()

            // Clean up stale lock files from old Moquette H2 format or crashed instances
            moquetteDir.listFiles()?.forEach { file ->
                if (file.name.endsWith(".h2") || file.name.endsWith(".lock") ||
                    file.name.endsWith(".lck") || file.name.endsWith(".trace.db")) {
                    file.delete()
                    Log.i(TAG, "Cleaned stale broker file: ${file.name}")
                }
            }
