...
class BleScanner(private val context: Context) {
    ...
    // ── RSSI filtering ───────────────────────────────────────────────
    // Two-pass: Kalman (per-sample noise) → EMA (inter-harvest jitter)
    // Based on AltBeacon + Nordic research: Q=0.01, R=3.0 best for zone detection

    private data class KalmanState(var estimate: Double, var errorCovariance: Double)
    private val kalmanStates = ConcurrentHashMap<String, KalmanState>()
    private val KALMAN_Q = 0.01   // process noise (tuned for zone detection)
    private val KALMAN_R = 3.0    // measurement noise

    private fun kalmanFilter(mac: String, rawRssi: Int): Double {
        val state = kalmanStates.getOrPut(mac) { KalmanState(rawRssi.toDouble(), 1.0) }
        val predictedError = state.errorCovariance + KALMAN_Q
        val gain = predictedError / (predictedError + KALMAN_R)
        state.estimate = state.estimate + gain * (rawRssi - state.estimate)
        state.errorCovariance = (1 - gain) * predictedError
        return state.estimate
    }

    // EMA second-pass — smooths inter-harvest jitter on published values
    // alpha=0.3: responsive to real movement, absorbs scan-to-scan noise
    private val emaValues = ConcurrentHashMap<String, Double>()
    private val EMA_ALPHA = 0.3

    private fun smoothedRssi(mac: String, rawRssi: Int): Int {
        val kalman = kalmanFilter(mac, rawRssi)
        val prev = emaValues[mac]
        val ema = if (prev == null) kalman else prev + EMA_ALPHA * (kalman - prev)
        emaValues[mac] = ema
        return ema.toInt()
    }
    ...
    private val scanCallback = object : ScanCallback() {
        ...
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            val rssi = smoothedRssi(mac, result.rssi)
