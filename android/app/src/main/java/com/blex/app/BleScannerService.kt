...
import com.blex.app.network.UdpDiscoveryManager
...
class BleScannerService : Service() {
    ...
    companion object {
        ...
        const val ACTION_RESTART = "com.blex.app.ACTION_RESTART_SERVICE"
        const val ACTION_RESPAWN_NOTIFICATION = "com.blex.app.ACTION_RESPAWN_NOTIFICATION"
    }
        ...
        notificationUpdateJob = serviceScope.launch {
            ...
            while (isActive) {
                try {
                    ...
                    val stateKey = "$currentIsScanning|$currentBrokerRunning|$remoteStatus|$currentBeacons|$currentMode"
        ...
        uiUpdateJob = serviceScope.launch {
            while (isActive) {
                ...
                ScanRepository.updateServiceStatus(
                    ...
                    startTime = serviceStartTime,
                    isBrokerRunning = embeddedBroker?.isRunning == true,
                    isBridgeConnected = mqttBridge?.isRemoteConnected == true
                )
    ...
    private fun buildNotification(
        ...
        val isLocalMode = settings.scannerProvisionMode == "local"
        val isBridgeUp = remoteStatus == "Up"
        val bleStatus = if (isScanning) "Active" else "Inactive"

        val (title, statusLine) = if (isLocalMode) {
            // Local mode: BLE | MQTT (broker) | Local (bridge to Pi)
            val mqttLabel = if (isBrokerRunning) "Up" else "Down"
            val localLabel = if (isBridgeUp) "Connected" else if (settings.remoteHost.isEmpty()) "--" else "Disconnected"
            val allGood = isScanning && isBrokerRunning && (settings.remoteHost.isEmpty() || isBridgeUp)
            val t = if (allGood) "BleX — Tracking Active" else "BleX — Degraded"
            val s = "BLE: $bleStatus | MQTT: $mqttLabel | Local: $localLabel"
            t to s
        } else {
            // Cloud mode: BLE | Cloud (bridge to DGX)
            val cloudLabel = if (isBridgeUp) "Connected" else if (settings.remoteHost.isEmpty()) "--" else "Disconnected"
            val allGood = isScanning && (settings.remoteHost.isEmpty() || isBridgeUp)
            val t = if (allGood) "BleX — Tracking Active" else "BleX — Degraded"
            val s = "BLE: $bleStatus | Cloud: $cloudLabel"
            t to s
        }
