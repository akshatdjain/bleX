...
### 2. [Backend API](./asset_api)
A professional FastAPI (Python) service for system management. Multi-tenant, schema-per-tenant PostgreSQL architecture.
- **RESTful API**: Handles zone management, asset registration, auth, and scanner assignments.
- **Persistence**: PostgreSQL-backed data store with schema-per-tenant isolation.
- **Dashboard**: React SPA (`beam_ui`) served via Caddy at `/beam`.
- **Scalability**: Designed to handle hundreds of scanners and thousands of assets.

Production: `https://sigmatic-asc.tech/asset` | `https://sigmatic-asc.tech/beam`
...
- `asset_api/`: FastAPI server — REST API, auth, multi-tenant.
- `beam_ui/`: React dashboard SPA.
- `master/`: Zone decision engine (MQTT → zone events).
- `provisioner/`: Provisioning logic and edge-node scripts.
- `legacy/`: Older Pi scripts and docs (reference only).
- `scripts/`: Helper scripts for deployment and maintenance.

## 🔧 Getting Started

1. **Backend**: `docker compose up -d` from the repo root (asset_api + beam_ui + postgres).
