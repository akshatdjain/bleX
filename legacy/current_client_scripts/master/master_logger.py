...
# ---------------- ADDITION (LOGGER ONLY) ----------------
from logger import TelemetryLogger

telemetry = TelemetryLogger(
    base_dir="logs",
    prefix="telemetry_master",
    use_utc=True
)
# --------------------------------------------------------
...
def handle_lost_assets():
    ...
    for asset_mac, last_seen in list(last_seen_registry.items()):
        ...
        push_fifo(event)
        print(f"[EXIT] {asset_mac} marked as EXIT")
...
def process_asset(asset_mac: str):
    ...
    # -------- ADDITION (telemetry only) --------
    state["deciding_rssi"] = round(proposed_rssi, 2)
    # -------------------------------------------
    ...
    if pending is None or pending["zone_id"] != proposed_zone:
        ...
        if ENABLE_DEBUG_LOGS:
            print(f"[DWELL-START] {asset_mac} → ZONE {proposed_zone}")
        return

    if now - pending["start_time"] < DWELL_TIME_SEC:
        if ENABLE_DEBUG_LOGS:
            print(f"[DWELL-WAIT] {asset_mac} → ZONE {proposed_zone}")
        return
    ...
    push_fifo(event)
    print(f"[ZONE] {asset_mac}: {last_zone} → {proposed_zone}")
...
def on_message(client, userdata, msg):
    ...
    raw_rssi = payload.get("rssi", {}).get("raw")
    ...
    # -------- ADDITION (telemetry only) --------
    state = ASSET_STATE[asset_mac]
    current_zone_raw = redis_get_last_zone(asset_mac)

    telemetry.log({
        "timestamp": payload.get("timestamp"),
        "scanner_id": scanner_id,
        "scanner_zone_id": zone_id,
        "asset_bluetooth_id": asset_mac,
        "asset_name": payload.get("asset_name"),
        "raw_rssi": raw_rssi,
        "kalman_rssi": kalman_rssi,
        "tx_power": tx_power,
        "deciding_rssi": state.get("deciding_rssi"),
        "current_zone_id": (
            int(current_zone_raw)
            if current_zone_raw not in (None, "EXIT")
            else None
        ),
        "state": "EXIT" if current_zone_raw == "EXIT" else "ZONE",
    })
    # -------------------------------------------
...
print("[MASTER] Zone-based master started")
