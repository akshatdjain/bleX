...
def _load_tenant_id() -> str:
    """Read tenant_id from ~/mqtt_config.json written by provisioner."""
    try:
        path = os.path.expanduser("~/mqtt_config.json")
        with open(path) as f:
            cfg = json.load(f)
        return cfg.get("tenant_id", "default")
    except Exception:
        return "default"
...
EDDYSTONE_UUID = "feaa"
BATTERY_SVC_UUID = "fff0"        # 0xFFF0 — last byte = battery %
...
# New topic format: ble/{tenant_id}/scanner/{mac}
# Backward compat: if tenant_id is "default", use old format ble/scanner/{mac}
...
def parse_battery(ad: AdvertisementData):
    """
    Extract battery % from Service UUID 0xFFF0 advertising data.
    Per your pseudocode: last byte of the service data payload.
      0x64 (100) = 100%
      0x65 (101) = USB / plugged-in power
    Returns int (0-101) or None if not present.
    """
    for svc_uuid, data in (ad.service_data or {}).items():
        # Match the standard 16-bit 0xFFF0 within the 128-bit UUID string
        if svc_uuid.lower().startswith("0000fff0"):
            if data and len(data) >= 1:
                return int(data[-1])  # 101 = USB, else % value
    return None
...
def detection_callback(device: BLEDevice, ad: AdvertisementData):
    ...
    if battery is not None:          # only overwrite when beacon broadcasts it
    ...
    while True:
        ...
        for mac in list(beacon_state.keys()):
            ...
            payload = {
                ...
                "battery": state.get("battery"),  # None when beacon doesn't broadcast it
            }
