...
def load_scanner_zone_map():
    global MAP_VERSION
    try:
        tid_param = f"&tenant_id={TENANT_ID}" if TENANT_ID else ""
        url = f"https://sigmatic-asc.tech/asset/api/runtime/scanner-zone-map/watch?version={MAP_VERSION}{tid_param}"
...
def health_push_loop():
    ...
    while True:
        ...
        try:
            health_suffix = f"?tenant_id={TENANT_ID}" if TENANT_ID else ""
            resp = requests.post(
                f"{HEALTH_API_BASE}/scanners/bulk{health_suffix}",
                json=scanner_health,
                timeout=5,
            )
        ...
        try:
            resp = requests.post(
                f"{HEALTH_API_BASE}/beacons/bulk{health_suffix}",
                json=beacon_health,
                timeout=5,
            )
