...
export default function AssetDetail() {
  ...
  const { data: assetLogs = [] } = useAssetHistory(assetId || "");
  ...
  if (!asset) {
    return (
      <div className="py-16 text-center text-muted-foreground">
        <p>Asset not found.</p>
        <Link to="/assets" className="text-primary text-sm mt-2 inline-block hover:underline">
          Back to assets
        </Link>
      </div>
    );
            ...
            {assetLogs.map((log, i) => (
              ...
              >
                <CardContent className="p-3 flex items-center gap-3 text-sm">
                  ...
                  <span className="truncate font-medium">{log.to_zone}</span>
                </CardContent>
              </Card>
            ))}
          </div>
